...
col3.metric("Exit time T with x²_T = 0", str(info.exit_time_T))
...
st.info(
    f"Household 2 exits at T = {info.exit_time_T}. "
    f"Therefore x²_T = 0 from period {info.exit_time_T} onward in the computed path."
)
...
column_order = [
    ...
    "x1_t_over_K_t",
    "x2_t_over_K_t",
    "saving_ratio_Kt_over_fKtminus1",
    "phase",
]
df = df[column_order]
...
The exit time reported by the dashboard is $T=n$, so $x_T^2=0$.
