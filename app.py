...
col3.metric("Exit time T where x²_T = 0", str(info.exit_time_T))
if info.block_length == 0:
    col4.metric("Active-block length", "0")
    col5.metric("Final-exit ratio z", "not used")
else:
    col4.metric("Active-block length", str(info.block_length))
    col5.metric("Final-exit ratio z", f"{info.final_exit_ratio_z:.8g}")

st.subheader("Capital paths")
capital_chart_df = df.set_index("t")[["K_t", "x1_t", "x2_t"]]
st.line_chart(capital_chart_df)

st.subheader("Capital shares")
share_chart_df = df.set_index("t")[["x1_t_over_K_t", "x2_t_over_K_t"]]
st.line_chart(share_chart_df)

st.subheader("Period-to-period changes in capital shares")
st.caption(
    "These are the differences "
    "x¹_{t-1}/K_{t-1} - x¹_t/K_t and x²_{t-1}/K_{t-1} - x²_t/K_t. "
    "The t = 0 row is blank because there is no previous period."
)
share_drop_chart_df = df.set_index("t")[["x1_share_drop_from_tminus1", "x2_share_drop_from_tminus1"]]
st.line_chart(share_drop_chart_df)
...
$$\frac{x_t^1}{K_t}, \qquad \frac{x_t^2}{K_t},$$

and the share changes

$$\frac{x_{t-1}^1}{K_{t-1}}-\frac{x_t^1}{K_t}, \qquad
\frac{x_{t-1}^2}{K_{t-1}}-\frac{x_t^2}{K_t}.$$

After household 2 exits at $T=n$, the patient-household continuation is
